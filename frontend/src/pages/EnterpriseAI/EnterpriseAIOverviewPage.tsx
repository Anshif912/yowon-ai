...
// Import sub-workspaces to embed directly as tabs
import CopilotWorkspacePage from './CopilotWorkspacePage'
...
export default function EnterpriseAIOverviewPage() {
  const [activeTab, setActiveTab] = useState<'overview' | 'copilot' | 'search' | 'predictions' | 'digital-twin'>('overview')
  ...
  return (
    <div className="flex-grow flex flex-col overflow-y-auto bg-[#05070a] min-h-full pb-12 select-none custom-scrollbar">
      {/* Page Header */}
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: activeTab === 'overview' ? 'Overview' : activeTab.toUpperCase() }
        ]}
        status={{ label: 'AI COGNITIVE ENGINE ACTIVE', type: 'success' }}
      />

      {/* Tabs navigation */}
      <div className="border-b border-white/[0.06] flex items-center gap-2 overflow-x-auto scrollbar-none px-6 sm:px-8 pb-0.5 mt-4">
        {[
          { id: 'overview', label: 'Consensus Overview', icon: Layers },
          { id: 'copilot', label: 'AI Copilot Chat', icon: MessageSquare },
          { id: 'search', label: 'Knowledge Graph Search', icon: Search },
          { id: 'predictions', label: 'Risk Predictions', icon: TrendingUp },
          { id: 'digital-twin', label: 'Digital Twin Simulator', icon: Brain }
        ].map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)}
            className={`flex items-center gap-2 px-4 py-2 border-b-2 font-mono text-xs transition-colors whitespace-nowrap cursor-pointer ${
              activeTab === tab.id
      ...
      <div className="flex-1 min-h-0">
                ...
                {quickLinks.map((link, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="group relative flex flex-col border border-white/5 bg-[#0b0c10]/80 rounded-xl p-6 hover:border-cyan-500/30 hover:-translate-y-1 transition-all duration-200 text-left cursor-pointer"
                    >
        ...
        {activeTab === 'copilot' && <CopilotWorkspacePage />}
        {activeTab === 'search' && <KnowledgeSearchPage />}
